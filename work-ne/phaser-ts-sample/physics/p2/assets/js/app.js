...
(function (FeatureSample) {
    var MainGame = /** @class */ (function (_super) {
        ...
        function MainGame(config) {
            ...
            _this.state.add('Sample', FeatureSample.PhysicsP2Sample);
...
(function (FeatureSample) {
    var ParticleWeatherSample = /** @class */ (function (_super) {
        ...
        ParticleWeatherSample.prototype.create = function () {
            ...
            this.createRain();
            this.makeButton('rain', 420, 40, function () {
                if (_this.rainEmitter.alive) {
                    _this.rainEmitter.kill();
                }
                else {
                    _this.rainEmitter.revive();
                }
            });
            this.makeButton('snow', 420, 80, function () {
            });
            this.makeButton('fog', 420, 120, function () {
                _this.addFog();
            });
        };
...
(function (FeatureSample) {
    var Vegetable = /** @class */ (function (_super) {
        ...
        function Vegetable(game) {
            var _this = this;
            var frame = game.rnd.between(3, 35);
            if (frame === 17) {
                frame = 1;
            }
            var x = game.rnd.between(100, 570);
            var y = game.rnd.between(0, 200);
            _this = _super.call(this, game, x, y, 'veggies', frame) || this;
            return _this;
        }
        return Vegetable;
    }(Phaser.Image));
    var Chilli = /** @class */ (function (_super) {
        __extends(Chilli, _super);
        function Chilli(game) {
            var _this = this;
            var x = game.rnd.between(100, 570);
            var y = game.rnd.between(0, 200);
            _this = _super.call(this, game, x, y, 'veggies', 17) || this;
            // Arcade Physics를 활성화 합니다.
        ...
        PhysicsArcadeSample.prototype.create = function () {
            ...
            for (var i = 0; i < 50; i++) {
                if (i < 30) {
                    ...
                    this.veggies.add(new Vegetable(this.game));
                }
                else {
                    //  But a chilli has a physics body
                    this.veggies.add(new Chilli(this.game));
                }
            }
            // 입력 키 처리
            this.cursors = this.input.keyboard.createCursorKeys();
