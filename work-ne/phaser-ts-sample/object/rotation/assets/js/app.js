...
(function (FeatureSample) {
    var MainGame = /** @class */ (function (_super) {
        ...
        function MainGame(config) {
            ...
            _this.state.add('Sample', FeatureSample.ObjectRotationSample);
...
var FeatureSample;
(function (FeatureSample) {
    var ObjectRotationSample = /** @class */ (function (_super) {
        __extends(ObjectRotationSample, _super);
        function ObjectRotationSample() {
            return _super !== null && _super.apply(this, arguments) || this;
        }
        ObjectRotationSample.prototype.preload = function () {
            this.game.load.image('arrow', 'assets/sprites/arrow.png');
            this.load.spritesheet('button', 'assets/sprites/flixel-button.png', 80, 20);
        };
        ObjectRotationSample.prototype.create = function () {
            var _this = this;
            this.sampleObject = this.game.add.sprite(200, 200, 'arrow');
            // 오브젝트 회전 시 기준 값을 지정합니다.
            this.sampleObject.pivot.x = 100;
            this.sampleObject.pivot.y = 100;
            this.makeButton('rotation', 420, 40, function () {
                _this.isRotaion = !_this.isRotaion;
            });
        };
        ObjectRotationSample.prototype.update = function () {
            if (this.isRotaion) {
                // 매 프레임 업데이트 될 때마다 지정된 pivot값을 기준으로 0.05 라디안 만큼 회전합니다.
                this.sampleObject.rotation += 0.05;
            }
        };
        ObjectRotationSample.prototype.render = function () {
            // 샘플 이미지의 중심점을 표시합니다.
            this.game.debug.geom(new Phaser.Point(this.sampleObject.x, this.sampleObject.y), '#ffff00');
        };
        // 테스트를 위한 버튼 생성
        ObjectRotationSample.prototype.makeButton = function (name, x, y, callback) {
            var button = this.add.button(x, y, 'button', callback, this, 0, 1, 2);
            button.name = name;
            button.scale.set(2, 1.5);
            button.smoothed = false;
        ...
        return ObjectRotationSample;
    }(Phaser.State));
    FeatureSample.ObjectRotationSample = ObjectRotationSample;
})(FeatureSample || (FeatureSample = {}));
var FeatureSample;
(function (FeatureSample) {
    var ParticleRainSample = /** @class */ (function (_super) {
        __extends(ParticleRainSample, _super);
        function ParticleRainSample() {
            return _super !== null && _super.apply(this, arguments) || this;
        }
        ParticleRainSample.prototype.preload = function () {
            this.load.image('sky', 'assets/sprites/sky5.png');
            this.load.spritesheet('rain', 'assets/sprites/rain.png', 17, 17);
        };
        ParticleRainSample.prototype.create = function () {
            this.add.image(0, 0, 'sky');
            this.rainEmitter = this.add.emitter(this.world.centerX, 0, 400);
            // 파티클 방출 영역을 지정합니다.
            this.rainEmitter.width = this.world.width;
            // 파티클 이미지의 회전 값을 지정합니다.
            this.rainEmitter.angle = 10;
            // 'rain' 파티클 입자를 생성합니다.
            this.rainEmitter.makeParticles('rain');
            // 파티클의 최소, 최대 스케일을 지정합니다.
            this.rainEmitter.minParticleScale = 0.1;
            this.rainEmitter.maxParticleScale = 0.5;
            // 파티클의 속도와 회전 값을 지정합니다.
        ...
        return ParticleRainSample;
    }(Phaser.State));
    FeatureSample.ParticleRainSample = ParticleRainSample;
})(FeatureSample || (FeatureSample = {}));
var FeatureSample;
(function (FeatureSample) {
...
(function (FeatureSample) {
    var TweenSample = /** @class */ (function (_super) {
        ...
        TweenSample.prototype.preload = function () {
            this.load.image('space', 'assets/sprites/starfield.jpg');
            this.load.image('logo', 'assets/sprites/phaser2.png');
        };
